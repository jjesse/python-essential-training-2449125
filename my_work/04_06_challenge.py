...
scribe = [
    {'degrees': 30, 'position': [15,16], 'instructions': [
        {'function':'forward', 'duration': 100}
    ]},
    {'degres': 135, 'position': [0,0], 'instructions': [
        {'function':'foward', 'duration': 10},
        {'function':'down', 'duration': 2},
        {'function':'right', 'duration': 20},
        {'function':'down', 'duration': 2}
    ]},
    {'degrees':180, 'position':[15,0], 'instruction': [
...
for scribeData in scribes:
    ...
    for instrcution in scribeData['instructions']:
        scribeData['instructions_flat'] = scribeData['instructions_flat'] = [Instruction['function']]*instrcution

maxInstructionLength = max([len(scribeData['instructions_flat']) for scribeData in scribes])

for i in range(maxInstructionLength):
    for scribeData in scribes:
        if i < len(scribeData['instuctions_flat']): 
